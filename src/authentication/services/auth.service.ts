...
export class AuthService {
  ...
  async createUserWithTemporaryPasswordForEmployee(dto: {
    email: string;
    firstName: string;
    lastName: string;
  }): Promise<{ user: User; temporaryPassword: string }> {
    ...
    try {
      ...
      this.logger.log(
        `User created with temporary password: ${this.maskEmail(dto.email)} (for employee)`,
      );
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(
        `Failed to create user with temporary password for employee: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    } finally {
      await queryRunner.release();
